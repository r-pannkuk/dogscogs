...
def get_modifier_strings(modifiers: typing.List[Modifier]):
    ...
    for modifier in modifiers:
        ...
        if modifier['operator'] == 'set':
            modifier_string = f"Set {TOKEN_MODIFIER_KEY} ({TOKEN_BONUS_TYPE}) to {modifier['value']}"
        if modifier['operator'] == 'add':
            modifier_string = f"+{modifier['value']} to {TOKEN_MODIFIER_KEY} ({TOKEN_BONUS_TYPE})"
        if modifier['operator'] == 'multiply':
            modifier_string = f"x{modifier['value']} to {TOKEN_MODIFIER_KEY} ({TOKEN_BONUS_TYPE})"

        if modifier['key'] == 'rolecolors':
            modifier_string.replace(TOKEN_MODIFIER_KEY, "Color Curses")
        if modifier['key'] == 'nyame':
            modifier_string.replace(TOKEN_MODIFIER_KEY, "Nyaming")
        if modifier['key'] == 'curse':
            modifier_string.replace(TOKEN_MODIFIER_KEY, "Nickname Curses")

        if modifier['type'] == 'attack':
            modifier_string.replace(TOKEN_BONUS_TYPE, "Attacking")
        if modifier['type'] == 'defend':
            modifier_string.replace(TOKEN_BONUS_TYPE, "Defending")
        if modifier['type'] == 'both':
            modifier_string.replace(TOKEN_BONUS_TYPE, "Attacking and Defending")
...
class BattlerRaceEmbed(discord.Embed):
    ...
    async def send(self, show_stats : bool = False) -> "BattlerRaceEmbed":
        ...
        modifier_strings = [f"- {string}" for string in get_modifier_strings(race['modifiers'])]

        if len(modifier_strings) == 0:
            modifier_strings = ["- None"]

        self.add_field(name="Modifiers", value='\n'.join(modifier_strings), inline=False)
        ...
        if show_stats:
            ...
            all_members : typing.Dict[int, BattleUserConfig] = await self.config.all_members(self.guild)
