...
class Nickname(commands.Cog):
    ...
    async def _calculate_cost(self, member: discord.Member, type: CurseType) -> int:
        ...
        if successive[type]['last_timestamp'] is not None and successive[type]['last_timestamp'] < start_of_week.timestamp():
            successive[type]['count'] = 0
            await self.config.member(member).successive.set(successive)

        cost += int(min(successive_count_max, successive[type]['count']) * cost * successive_cost_increase)
