...
class BetListPaginatedEmbed(PaginatedEmbed):
    ...
    def __init__(
        ...
        *args,
        config: Config,
        ctx: commands.GuildContext,
        filter: typing.Callable[[BetConfig], bool] = lambda x: x["state"]
        in ["open", "closed", "config"],
        **kwargs,
    ):
        ...
        async def get_page(index: int) -> typing.Tuple[discord.Embed, int]:
            active_bets: typing.Dict[str, BetConfig] = await self.config.guild(
                self.guild
            ).active_bets()
            ...
            return await BetEmbed(
                config=self.config, ctx=self.ctx, bet_config_id=self.bet_config["id"]
            ...
            self.bet_list_select: OnCallbackSelect = OnCallbackSelect(
                ...
                options=[
                    discord.SelectOption(
                        ...
                        + (
                            f" || Edited: {datetime.fromtimestamp(config['last_edited_at'], tz=TIMEZONE).strftime('%m-%d-%Y %I:%M %p (%Z)')}"
                            if config["last_edited_at"] is not None
                            else f" || Created: {datetime.fromtimestamp(config['created_at'], tz=TIMEZONE).strftime('%m-%d-%Y %I:%M %p (%Z)')}"
                        ),
                    )
    ...
    async def edit_page(self) -> None:
        ...
        await self.message.edit(
            content=f"Found {self.total_pages} Bets.\n`{self.index + 1}/{self.total_pages}`",
            view=self,
        )
...
class ChooseWinnerSelect(discord.ui.Select):
    def __init__(
        self, *args, parent_view: discord.ui.View, bet_config: BetConfig, **kwargs
    ):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        chosen_options = [
            self.bet_config["options"][int(value)]["option_name"]
            for value in self.values
        ]
        await interaction.response.send_message(
            f"Choosing: `{'`, `'.join(chosen_options)}`\nAre you sure?",
            ephemeral=True,
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.bet_config["author_id"]:
            await interaction.response.send_message(
                "❌ ERROR: You are not the author of this bet.",
                ephemeral=True,
                delete_after=5,
            )
            return False
...
class BetAdministrationConfigModal(discord.ui.Modal):
    ...
    def __init__(self, *args, bet_config: BetConfig, started: bool = False, **kwargs):
        ...
        self.bet_options: discord.ui.TextInput = discord.ui.TextInput(
            ...
            default=", ".join(
                [option["option_name"] for option in bet_config["options"]]
            ),
            row=3,
            required=True,
            style=discord.TextStyle.long,
    ...
    def _get_options(self) -> typing.List[str]:
        return [
            option.strip().strip("\"'") for option in self.bet_options.value.split(",")
        ]
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if len(parsed_options) < 2:
            await interaction.response.send_message(
                "You must have at least 2 options.", ephemeral=True, delete_after=5
            )
            return False
        ...
        if len(distinct_options) != len(parsed_options):
            await interaction.response.send_message(
                "❌ ERROR: Options must be distinct.", ephemeral=True, delete_after=5
            )
            return False
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        self.bet_config["options"] = [
            {"id": i, "option_name": option}
            for i, option in enumerate(self._get_options())
        ]

        await interaction.response.send_message(
            "Bet configuration updated.", ephemeral=True, delete_after=5
        )
    ...
    async def on_error(self, interaction: discord.Interaction, exception: Exception) -> None:  # type: ignore
        await interaction.response.send_message(
            str(exception), ephemeral=True, delete_after=15
        )
...
class BetButton(discord.ui.Button):
    ...
    def _get_better(self, better_list: typing.List[Better], member: discord.Member):
        found_betters = [
            better for better in better_list if better["member_id"] == member.id
        ]
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        message = await interaction.followup.send(
            f"💰 You have placed a bet of `{amount}` on `{config['options'][self.option_id]['option_name']}` (Total: `{better['bet_amount']}`)",
            ephemeral=True,
            wait=True,
        )
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if existing_bet is not None and existing_bet["bet_option_id"] != self.option_id:
            found_option = next(
                (
                    ...
                    if option["id"] == existing_bet["bet_option_id"]
                ),
                None,
            )
            ...
            raise Exception(
                f"You have already placed a bet on `{found_option['option_name']}`"
            )
        ...
        if balance < config["minimum_bet"]:
            raise Exception(
                f"You don't have enough coins in your balance to place a bet.\nMinimum: `{config['minimum_bet']}`\nBalance: `{balance}`"
            )
...
class BetAdministrationView(discord.ui.View):
    ...
    async def _author_or_elevated_check(self, interaction: discord.Interaction):
        role_ids = await self.config.guild(self.guild).allowed_role_ids()
        return (
            isinstance(interaction.user, discord.Member)
            and (
                interaction.user.guild_permissions.manage_roles
                or any(role.id in role_ids for role in interaction.user.roles)
            )
        ) or await self._author_check(interaction)

    async def _regenerate_message(self):
        await self.generate()
        await self.original_message.edit(
            embed=await BetEmbed(
                config=self.config, ctx=self.ctx, bet_config_id=self.bet_config_id
            ).generate(),
            view=self,
        )

    @discord.ui.button(
        custom_id=f"edit_config:",
        label="Config",
    ...
    async def edit_config(
        ...
        modal = BetAdministrationConfigModal(
            bet_config=config, started=config["state"] != "config"
        )
    ...
    async def toggle_open(
        ...
        if config["state"] == "resolved" or config["state"] == "cancelled":
            await interaction.response.send_message(
                "❌ ERROR: Bet is already resolved or cancelled.",
                ephemeral=True,
                delete_after=5,
            )
            return
        ...
        config["state"] = (
            "open"
            if config["state"] == "closed" or config["state"] == "config"
            else "closed"
        )
        ...
        await interaction.response.send_message(
            f"Betting is now {config['state']}.", ephemeral=True, delete_after=5
        )
    ...
    async def add_pool(
        ...
        if not await self._author_or_elevated_check(interaction):
            return
        ...
        total = (
            sum(better["bet_amount"] for better in config["betters"])
            + config["base_value"]
        )
    ...
    async def resolve(
        ...
        await interaction.response.send_message(
            "Select the winning option:", ephemeral=True, view=action_bar
        )
        ...
        async with self.config.guild(self.guild).active_bets.get_lock():
            ...
            config = active_bets[str(self.bet_config_id)]
            config["winning_option_id"] = action_bar.select_winner.bet_config[
                "winning_option_id"
            ]
            ...
            if bet_total == 0:
                message = await interaction.followup.send(
                    "No bets were placed.", ephemeral=True, wait=True
                )
            ...
            winning_option = next(
                (
                    option
                    for option in config["options"]
                    if option["id"] == config["winning_option_id"]
                ),
                None,
            )
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(
            f"Bets have been cancelled.", ephemeral=True, delete_after=5
        )
    ...
    async def check_bet(
        ...
        better = next(
            (
                better
                for better in config["betters"]
                if better["member_id"] == interaction.user.id
            ),
            None,
        )
        ...
        if better:
            bet_option = next(
                (
                    option
                    for option in config["options"]
                    if option["id"] == better["bet_option_id"]
                ),
                None,
            )
            ...
            if config["state"] == "resolved":
                if better["bet_option_id"] == config["winning_option_id"]:
                    ...
                    results_msg = (
                        f"💸 You bet `{better['bet_amount']}` and won `{total_winnings}` "
                        + (
                            f"(+{total_winnings/better['bet_amount'] - 1:.2%}) "
                            if total_winnings > better["bet_amount"]
                            else ""
                        )
            ...
            await interaction.response.send_message(
                results_msg, ephemeral=True, delete_after=15
            )
        else:
            await interaction.response.send_message(
                "You have not placed a bet on this pool.",
                ephemeral=True,
                delete_after=5,
            )

    @discord.ui.button(
        label="Check Winners", style=discord.ButtonStyle.secondary, row=2
    )
    async def list_winners(
        ...
        if config["winning_option_id"] is None:
            await interaction.response.send_message(
                "Winning option has not been selected.", ephemeral=True, delete_after=5
            )
            return
        ...
        betters_list = sorted(
            config["betters"],
            key=lambda x: (
                x["bet_option_id"] == config["winning_option_id"],
                x["bet_amount"],
            ),
            reverse=True,
        )
        ...
        for i, better in enumerate(betters_list):
            ...
            if i < MAX_WINNERS_DISPLAY_LENGTH:
                ...
                results_msg += (
                    f" on `{config['options'][better['bet_option_id']]['option_name']}`"
                )
                results_msg += (
                    f" and won `{total_winnings}` "
                    + f"(+{total_winnings/better['bet_amount'] - 1:.2%}) "
                    if total_winnings > better["bet_amount"]
                    else ""
                )
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception, item: Item) -> None:  # type: ignore[override]
        if interaction.response.is_done():
            message = await interaction.followup.send(
                f"❌ ERROR: {error}", ephemeral=True, wait=True
            )
            await message.delete(delay=15)

        else:
            await interaction.response.send_message(
                f"❌ ERROR: {error}", ephemeral=True, delete_after=15
            )
