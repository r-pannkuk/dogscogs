...
class PointsPassiveConfigurationEmbed(discord.Embed):
    ...
    async def collect(self):
        ...
        self.passive_channels = [c for c in [self.guild.get_channel(channel_id) for channel_id in self.passive_channel_ids] if c is not None]
        ...
        self.passive_response_multiplier = await self.group.passive_response_multiplier()
        self.passive_response_jackpot_chance = await self.group.passive_response_jackpot_chance()
        self.passive_response_jackpot_multiplier = await self.group.passive_response_jackpot_multiplier()
        ...
        self.add_field(
            ...
            f"__Triggering Channels__: {', '.join([channel.mention for channel in self.passive_channels]) if len(self.passive_channels) > 0 else 'All'}\n" +
...
class NumberPromptModal(discord.ui.Modal):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if value < self.min or value > self.max:
            raise ValueError(f"Please enter a number between {self.min} and {self.max}.")
        
        return True
    
    async def on_error(self, interaction: discord.Interaction, exception: Exception) -> None: # type: ignore
        await interaction.response.send_message(str(exception), ephemeral=True, delete_after=20)
...
class PointsPassiveConfigurationView(_ConfigurationView):
    ...
    async def edit_award_amount(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user != self.author:
            return
    ...
    @discord.ui.button(label="Edit Daily Limit", style=discord.ButtonStyle.secondary, row=0)
    async def edit_max_awards(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user != self.author:
            return
    ...
    async def edit_channels(self, interaction: discord.Interaction, select: discord.ui.ChannelSelect):
        if interaction.user != self.author:
            return
    ...
    @discord.ui.button(label="Edit Bonus Chance", style=discord.ButtonStyle.secondary, row=2)
    async def edit_bonus_chance(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user != self.author:
            return
    ...
    @discord.ui.button(label="Edit Bonus Multiplier", style=discord.ButtonStyle.secondary, row=2)
    async def edit_bonus_multiplier(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user != self.author:
            return
    ...
    @discord.ui.button(label="Edit Jackpot Chance", style=discord.ButtonStyle.secondary, row=4)
    async def edit_jackpot_chance(self, interaction: discord.Interaction, _button: discord.ui.Button):
        if interaction.user != self.author:
            return
    ...
    @discord.ui.button(label="Edit Jackpot Multiplier", style=discord.ButtonStyle.secondary, row=4)
    async def edit_jackpot_multiplier(self, interaction: discord.Interaction, _button: discord.ui.Button):
        if interaction.user != self.author:
            return
